...
public abstract class PdfViewerActivity extends Activity {
	...
	private final static int MENU_PRINT   = 8;
...
//		private int totalpages = mPdfFile.getNumPages(); 
		...
		public void onLayout(PrintAttributes oldAttributes,
			      ...
			      Bundle metadata) {
			Log.d("Alex", "onLayout is called!"); 
			myPdfDocument = new PrintedPdfDocument(this.context, newAttributes);
			...
			Log.d("Alex", "totalpages here is: "+totalpages);     
			if (totalpages > 0) {
		...
		public void onWrite(final PageRange[] pageRanges,
			...
			Log.d("Alex", "onWrite is called!");
			for(int index=0; index<pageRanges.length; index++) {
				Log.d("Alex", "pageRanges:["+index+"] is " +pageRanges[index]);	
			} 
			for (int i = 0; i < totalpages; i++) {
				if (pageInRange(pageRanges, i)) {
					PageInfo newPage = new PageInfo.Builder(pageWidth, pageHeight, i).create();
					Log.d("Alex", "page number is: "+newPage.getPageNumber()); 
		...
		private boolean pageInRange(PageRange[] pageRanges, int page) {
			Log.d("Alex", "page is: "+page); 
			for (int i = 0; i<pageRanges.length; i++) {
				Log.d("Alex", "start is: "+pageRanges[i].getStart()+" end is: "+pageRanges[i].getEnd()); 
        ...
        if (mOldGraphView != null) {
	        ...
	        if (intent != null) {
	        	if ("android.intent.action.VIEW".equals(intent.getAction())) {
        			...
        			jobName = pdffilename; 
        			Log.d("Alex", "pdffilename is: "+pdffilename); 
	        	}
	        	else {
	                pdffilename = getIntent().getStringExtra(PdfViewerActivity.EXTRA_PDFFILENAME);
	                jobName = pdffilename; 
	                Log.d("Alex", "intent is null, pdffilename is: "+pdffilename); 
	        	}
	        }
...
//			        	Debug.startMethodTracing("andpdf");
			        	totalpages=mPdfFile.getNumPages(); 
			        	showPage(page, zoom);
...
//    		printDocumentOneByOne(); 
    ...
    private void printDocumentOneByOne() {
	    ...
	    photoPrinter.printBitmap("Text", myBitmap);
	}
...
//			tvSpacer.setHeight(height);
    ...
    private void showPage(int page, float zoom) throws Exception {
    	Log.d("Alex", "finally here? page is: "+page); 
    	...
    	try {
	        ...
	        Log.d("Alex", "mPdfPage is? "+mPdfPage); 
	        if (mPdfPage == null || mPdfPage.getPageNumber() != page) {
	        	mPdfPage = mPdfFile.getPage(page, true);
	        }
	        Log.d("Alex", "getPageNumber? "+mPdfPage.getPageNumber());
